...
describe('parseSourceToSchema', () => {
  ...
  it('calls all subsections', () => {
    ...
    expect(gen.genericInfo).toHaveBeenCalledWith(SOURCE.source.id, INTL, SOURCE_TYPE, SOURCE.applications);
    expect(app.applicationsFields).toHaveBeenCalledWith(SOURCE.applications, SOURCE_TYPE, APP_TYPES, SOURCE.authentications);
    expect(end.endpointFields).toHaveBeenCalledWith(SOURCE_TYPE);
    expect(titleField.default).toHaveBeenCalledWith(SOURCE.applications, SOURCE_TYPE, APP_TYPES, INTL, SOURCE.authentications);
  });
  ...
  it('returns filtered fields', () => {
    ...
    gen.genericInfo = jest.fn().mockImplementation(() => [FIELD1, undefined]);
    ...
    expect(result).toEqual({
      fields: [...FILTERED_FIELDS, titleField.default(SOURCE.applications, SOURCE_TYPE, APP_TYPES, INTL)],
    });
  });
});
