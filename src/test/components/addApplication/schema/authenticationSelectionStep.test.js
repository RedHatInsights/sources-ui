...
describe('authenticationSelectionStep', () => {
    ...
    const selectionStep = authenticationSelectionStep(
        SOURCE_TYPE,
        APP_TYPE,
        INTL,
        AUTH_VALUES
    ...
    it('nextstep is first step of the authentication', () => {
        expect(selectionStep.nextStep({ values: { application: { application_type_id: '456' }, authtype: 'arn' } }))
        .toEqual('amazon-456-arn');
    });

    it('nextstep is authentication selection when there are auths available', () => {
        expect(selectionStep.nextStep({ values: { application: { application_type_id: '456' }, authtype: 'password' } }))
        .toEqual('selectAuthentication');
    });

    it('nextstep is undefined when no authtype', () => {
        expect(selectionStep.nextStep({ values: { application: { application_type_id: '456' } } }))
        .toEqual(undefined);
    });
});
