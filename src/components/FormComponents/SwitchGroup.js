      ...
      <Stack hasGutter>
        {filteredOptions.map((option) => (
          <StackItem key={option.value}>
            ...
            />
            {option.description && (
              <div className="pf-v6-c-switch pf-v6-u-mt-sm">
                <span className="pf-v6-c-switch__toggle src-m-wizard-hide-me" />
                <div className="pf-v6-c-switch__label src-c-wizard--switch-description">{option.description}</div>
              </div>
            )}
          </StackItem>
        ))}
