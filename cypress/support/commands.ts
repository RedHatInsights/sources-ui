...
Cypress.Commands.add('filterBy', (filterText) => {
  cy.get('button[aria-label="Conditional filter toggle"]').click();
  cy.contains('[class*="-c-menu__item"]', filterText).click();
});

Cypress.Commands.add('filterSelect', (filterText) => {
  cy.get('[data-ouia-component-id="ConditionalFilter"] button[class*="-c-menu-toggle"]').click();
  cy.contains('[class*="-c-menu__list-item"]', filterText).click();
});
